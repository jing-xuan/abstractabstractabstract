...
From the fact that the abstract interpreter loops and never reaches the $DONE$ instruction, we can conclude that the concrete machine will never reach the $DONE$ instruction as well, this follows from the soundness of our interpreter and shows that the concrete machine will never terminate on this program.
...
% Can add screenshots and explanation on features and how they were implemented.
...
The soundness of the abstract interpreter ensures that these optimisations do not change program behaviour. If a value is received or returned by a function in the concrete, then it would be represented by some abstract state. Although the converse is not true and the abstract interpreter may show values that can never be reached by the concrete machine.

% Can add examples here
